...
namespace VideoRecorder
{
    public class AviCapture
    {
        ...
        public static extern bool capGetDriverDescription(int wDriverIndex, StringBuilder lpszName, int cbName,
        ...
        public const int
            WM_CAP_SEQUENCE_NOFILE =
                WM_START + 63; //使用WM_CAP_SEUENCE_NOFILE消息（capCaptureSequenceNoFile宏），可以不向磁盘文件写入数据。该消息仅在配合回调函数时有用，它允许你的应用程序直接使用音视频数据。
...
                                                     但是如果你是为了真，你必须要在程序中处理一些潜在的操作，因为当视频捕获时，其他操作并没有被屏蔽。*/
        ...
        {
            [MarshalAs(UnmanagedType.U2)] public UInt16 wDeviceIndex; //捕获驱动器的索引值，该值可以由0到9变化。
            [MarshalAs(UnmanagedType.Bool)] public bool fHasOverlay; // 视频叠加标志，如果设备支持视频叠加这该位是真。
            [MarshalAs(UnmanagedType.Bool)] public bool fHasDlgVideoSource; //视频资源对话框标志位，如果设备支持视频选择、控制对话框，该值为真。
            [MarshalAs(UnmanagedType.Bool)] public bool fHasDlgVideoFormat; //视频格式对话框标志位，如果设备支持对视频格式对话框的选择，该位真。
            [MarshalAs(UnmanagedType.Bool)] public bool fHasDlgVideoDisplay; //视频展示对话框标志位，如果设备支持对视频捕获缓存区的重新播放，该位是真。

            [MarshalAs(UnmanagedType.Bool)] public bool fCaptureInitialized; //捕获安装标志位，如果捕获驱动器已经成功连接，该值为真。

            //[MarshalAs(UnmanagedType.Bool)]
            public bool fDriverSuppliesPalettes; //驱动器调色板标志位，如果驱动器能创建调色板，则该位是真。
            [MarshalAs(UnmanagedType.I4)] public int hVideoIn;
            [MarshalAs(UnmanagedType.I4)] public int hVideoOut;
            [MarshalAs(UnmanagedType.I4)] public int hVideoExtIn;
            [MarshalAs(UnmanagedType.I4)] public int hVideoExtOut;
        }
        //=========================================================================================================================================
